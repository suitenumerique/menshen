...
class TokenExchangeView(APIView):
    ...
    def post(self, request):  # noqa: PLR0911, PLR0912, PLR0915
        ...
        # XXX : manage the user impersonation case ???? another view ?  # noqa: FIX003
        try:
            ...
            user_info = introspection_backend.get_user_info_with_introspection(subject_token)
            if introspection_backend.token_origin_audience != service_provider.audience_id:
                logger.error(
        ...
        if user_info.get(settings.OIDC_RS_AUDIENCE_CLAIM) != service_provider.audience_id:
            logger.error(
        ...
        subject_scopes = set(subject_token_scope.split()) if subject_token_scope else set()
        ...
        if not requested_scopes_and_actions:
            ...
            requested_actions = {s for s in requested_scopes_and_actions if s.startswith("action:")}
        ...
        if requested_actions:
            ...
            if requested_scopes:
                # TODO  # noqa: FIX002
                # allow distinct scope (ie not contained in action) to be requested.  # noqa: FIX002
                logger.warning(
        ...
        for scope_grant in ScopeGrant.objects.filter(
            ...
            existing_access[f"{scope_grant.audience_id}:{scope_grant.granted_scope}"] = scope_grant
            ...
            grants_dict.setdefault(audience_id, []).append(
                {
                    ...
                    "throttle": (
                        {"rate": scope_grant.throttle_rate} if scope_grant.throttle_rate else {}
                    ),
                }
            )
        ...
        for action_perm in action_permissions:
            ...
            if (
                action_perm.required_source_scope
                and action_perm.required_source_scope not in subject_scopes
            ):
                ...
                grants_dict.setdefault(audience_id, []).append(
                    {
                        ...
                        "throttle": ({"rate": ag.throttle_rate} if ag.throttle_rate else {}),
                    }
                )
        ...
        token_type = validated_data.get("requested_token_type") or TokenTypeChoices.ACCESS_TOKEN

        # Determine expires_in from rule or default
        expires_in = (
            validated_data.get("expires_in")
            # XXX or int(rule.exchanged_token_duration.total_seconds())  # noqa: FIX003
