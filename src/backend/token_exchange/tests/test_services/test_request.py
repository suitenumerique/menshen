...
def test_request_service_instantiation_with_base_request(source_service):
...
def test_request_service_rules_cached_property(source_service, target_service):
    ...
    assert request_service.rules.count() == 1
    rule = request_service.rules.get()
...
def test_request_service_validate_target_only_unknown_audiences(source_service, monkeypatch):
    ...
    with pytest.raises(
        TokenExchangeInvalidTargetError,
        match=re.escape("Only unknown audience(s) requested: service:target"),
    ):
        request_service._validate_target()


def test_request_service_validate_target_with_unknown_audience(source_service):
    ...
    with pytest.raises(
        TokenExchangeInvalidTargetError,
        match=re.escape("Unknown audience(s) requested: service:foo"),
    ):
        request_service._validate_target()
...
def test_request_service_introspect_subject_token_suspicious_origin_audience(
    ...
    Invalid introspection backend token_origin_audience case.
    ...
    def mock_user_info(self, _):
        self.token_origin_audience = "service:suspicious"
        return {}
    ...
    assert (
        "Introspected token origin is different from requesting service: "
...
def test_request_service_introspect_subject_token_missing_identity(
    ...
    with (
        pytest.raises(
            TokenExchangeIntrospectionError,
            match=r"Subject token introspection failed to provide identity \(sub or email\)",
        ),
        caplog.at_level(logging.WARNING),
    ):
        request_service._introspect_subject_token()

    assert "Introspection response missing both 'sub' and 'email'" in caplog.messages


def test_request_service_validate_pure_scopes_from_user_info_scope(source_service):
    """
    Test the request service validate pure scopes method with scopes from the user info.

    Without scopes in the request, we fallback to user scopes and it should work perfectly
    as the user has "openid target:read target:write" it its scope (from subject token
    introspection).
    ...
    assert sorted(request_service.grants, key=lambda g: g.scope) == [
        MenshenJWTGrantClaim(audience_id="service:target", scope=scope, throttle=None)
        for scope in sorted(request_service.granted_scopes)
    ]
...
def test_request_service_validate_scope_action_from_request(source_service):
    """
...
def test_request_service_validate_scope_action_from_request_with_missing_required_source_scope(
    source_service, target_service, source_target_rule
):
    """
    Test the request service validate scope action method with scopes from the request.

    In this case, one extra TokenExhangeActionPermission has an additional required source scope
    and the user does not have this scope.
...
def test_request_service_generate_exchange_token_jwt_no_sub_claim(
    ...
    with pytest.raises(
        TokenExchangeIssuingError,
        match="A subject claim is required to generate an exchange token.",
    ):
...
def test_request_service_generate_exchange_token_jwt_invalid_token(source_service, monkeypatch):
    ...
    with pytest.raises(TokenExchangeIssuingError, match="An error occured while issuing JWT."):
        request_service._generate_exchange_token(
            AllowedSubjectTokenTypeEnum.JWT,
            scope="target:write",
            expires_in=3600,
...
@pytest.mark.parametrize(
    "token_type", [AllowedSubjectTokenTypeEnum.ACCESS_TOKEN, AllowedSubjectTokenTypeEnum.JWT]
)
def test_request_service_generate_exchange_response(source_service, settings, token_type):
